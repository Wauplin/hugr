...
Every op records `OpMeta` when it ends. Cost is just *one* field — **timing matters at least as much**: `started_at`/`ended_at` give wall-clock latency per op (which model call was slow, how long a tool ran), useful for observability, scheduling, and debugging. `OpMeta` holds `{ started_at, ended_at, model: Option<ModelSelector>, usage: Option<Usage>, extra: Value }`, where `extra` is an opaque bag (provider request-id, cache-hit info, retry count, …) the brain stores but never interprets (narrow-waist, §2.4). Because it lives on the log record, latency and cost are queryable from the trace itself — no side table — and aggregate per op, per model selector, or per sub-agent.

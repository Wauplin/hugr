...
**The store.** The default store is shared at `~/.huggr/blobs` (override with `HUGGR_BLOB_STORE`, or `HUGGR_HOME/blobs`). Objects are keyed by SHA-256 and installed atomically as read-only files, so identical content deduplicates to one object no matter how many agents produce it. An inbound object is checked against its content address before materialization. When the store and scratchpad share a filesystem, a verified `sha256` blob is hardlinked into the scratch rather than copied: a parent granting `[tools.agent.<name>]` forwards `sha256` refs as `--blob sha256:<hash>` and no second copy crosses the process boundary (see [composition and cost](../guides/compose-agents.md)).

...
You'll grant one Hugr agent to another as a tool, hand a blob from parent to child without copying a byte, file feedback on the child's trace, and read the bill with `hugr stats`. By the end you'll know how agents compose and how their cost folds upward — the cleanup-route tutorial (08) builds on this.

This assumes you've done [01](01-first-agent-cli.md) (you have a built binary) and [02](02-typed-responses-and-hooks.md) (you know the runtime-arg pattern). The composition model is specified in [`ARCHITECTURE.md`](../../ARCHITECTURE.md) §8; this tutorial is the worked example.
...
`<name>` is `weather`, so the parent gets a capability named `agent_weather`. The parent's model sees it as an ordinary tool whose args are an [`Ask`](../../ARCHITECTURE.md) — a `question`, an optional `trace_id` to resume the child, and optional `blobs`:
...
- **Privileges compose downward only.** The child runs under its **own** manifest — its own tool jail, tiers, limits. Granting it never leaks the parent's capabilities into it; an agent with `[tools.fs_read]` cannot reach into the child's scratch.
...
Feedback is the one asynchronous back-channel — a way for a caller to record, *beside* an immutable trace, that an answer helped or didn't. It is never read during a live ask (see [`ARCHITECTURE.md`](../../ARCHITECTURE.md) Part IV); it's for offline analysis (tutorial 08).
...
Every number in `hugr stats` is already in the traces — `OpEnded` carries per-op cost/tokens/timing — the command just folds them. From the agent crate:
...
Per-tier, per-tool, and duration percentiles come for free from the same fold; see `crates/hugr-agent/src/analytics.rs` and `crates/hugr-toolkit/src/stats.rs` for the exact shape.

## Next

You've composed agents and read their cost. The traces those runs leave behind are replayable bit-for-bit — that's the debugging surface, and it's where an offline agent can mine them for improvement. On to [Traces, replay, and debugging](08-traces-replay-debugging.md).
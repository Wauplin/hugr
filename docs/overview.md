...
- **Self-contained to ship.** `huggr build` produces one standalone CLI binary per agent; the same binary is an MCP server via `--mcp-serve`. There is exactly one artifact kind.
...
- **Sandboxed by default.** A huglet gets a private scratchpad and exactly the tools it declares. Blob exchange with the caller is explicit.
...
- **One way to do each thing.** One artifact kind, one run path per stage (dev: `huggr run`; ship: the built binary), one external-tool escape hatch (MCP), one trace format. Breaking changes are acceptable; there is no backward-compatibility ceremony.

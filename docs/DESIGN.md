...
- **Compaction is itself a model call** → projection stays pure/synchronous (reads existing summaries only); compaction is a **separate `"summarizer"` model op** whose result is appended as a summary `Record` the next projection consumes. Adds a compaction sub-loop. (ARCHITECTURE §3.4)

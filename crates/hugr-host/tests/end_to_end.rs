...
async fn multi_turn_session_with_tool_round_trip() {
    ...
    assert_eq!(name, "echo");
    assert_eq!(result["stdout"], json!("hello-from-hugr"));
...
async fn run_chunked(answer: &str, chunk_size: usize) -> (Vec<hugr_core::LogEntry>, String) {
    let capture = Capture::default();
...
async fn resume_after_crash_starts_quiescent() {
    ...
    let saved = hugr_host::Trace::new(events, Vec::new(), Some(1));

...
use hugr_core::{Decision, DoneReason, ModelSelector, OpId, OutputEvent, Usage, Value};
...
/// measured host-side (`Instant`); `hugr-core` stays clock-free.
...
pub struct StdoutFrontend {
    ...
    /// to a head + "… +N lines" summary. Defaults from `HUGR_FULL_OUTPUT`.
    full_output: bool,
...
impl StdoutFrontend {
    ...
    fn tool_end_lines(&self, name: &str, result: &Value, is_error: bool) -> Vec<String> {
        ...
        if hidden > 0 {
            let note = format!("    … +{hidden} lines (set HUGR_FULL_OUTPUT=1 to expand)");
...
mod tests {
    ...
    fn tool_result_collapses_by_default() {
        ...
        assert!(
            joined.contains("HUGR_FULL_OUTPUT"),
    ...
    /// `with_full_output(true)` (the `HUGR_FULL_OUTPUT` toggle) shows everything.
    #[test]
    ...
    /// `HUGR_FULL_OUTPUT` parsing: truthy vs falsy values.
    #[test]

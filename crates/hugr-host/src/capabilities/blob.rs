...
//!
//! The disk-backed [`BlobStore`] itself lives in `hugr-replay` (the host-side
//! persistence crate) so it shares the exact [`BlobRef`](hugr_replay::BlobRef)
...
///   `{ "hash", "len", "media" }` (a [`BlobRef`](hugr_replay::BlobRef)).

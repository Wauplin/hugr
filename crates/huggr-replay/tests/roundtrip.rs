...
fn write_then_load_roundtrips_a_session_to_disk() {
    ...
    let dir = std::env::temp_dir().join(format!("huggr-replay-test-{}", std::process::id()));
...
fn recorded_commands_roundtrip() {
    use huggr_core::{Command, DoneReason};
    let commands = vec![
        Command::Checkpoint,
        Command::Done {
...
fn rejects_unsupported_future_version() {
    ...
    assert!(
        matches!(err, huggr_replay::TraceError::UnsupportedVersion { .. }),

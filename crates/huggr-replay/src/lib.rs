...
/// A saved Huggr session: a versioned container over the ordered event stream,
...
pub struct Trace {
    ...
    /// The session's [`TurnPolicy`](huggr_core::TurnPolicy) configuration, as an
...
impl Trace {
    ...
    /// produced by serializing its [`TurnPolicy`](huggr_core::TurnPolicy)), so

...
//! `huggr-agent` API, they must agree on the agent card, ask statuses, and
//! trace-lineage relationships.
//!
//! The whole suite is `#[ignore]`d: it compiles a real cli/mcp binary, which
//! is slow. Run it with `cargo test -p huggr-toolkit --test conformance --
//! --ignored`. It is the gate for `huggr build` changes.
//!
...
fn every_surface_agrees_on_the_scenario() {
    let base = std::env::temp_dir().join(format!("huggr-conf-{}", std::process::id()));

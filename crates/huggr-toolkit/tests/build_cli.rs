...
//! `huggr_toolkit::build::build_cli` and runs the produced binary; it is
//! `#[ignore]`d because compiling a detached shim crate is slow.

use std::path::{Path, PathBuf};

use huggr_agent::{Ask, STATUS_ERROR};
use huggr_toolkit::bundle;
use huggr_toolkit::manifest::AgentDefinition;
use huggr_toolkit::runtime::build_agent;
use huggr_toolkit::scaffold::{Template, scaffold_files};
...
fn scaffold_bundle(name: &str, template: Template) -> (Vec<u8>, PathBuf) {
    let src = std::env::temp_dir().join(format!("huggr-bcli-src-{name}-{}", std::process::id()));
...
fn unpack_home(bytes: &[u8], tag: &str) -> PathBuf {
    let home = std::env::temp_dir().join(format!("huggr-bcli-home-{tag}-{}", std::process::id()));
...
/// dependency tree and is slow. Run with `cargo test -p huggr-toolkit --
/// --ignored real_build`.
#[test]
#[ignore = "invokes cargo build; slow"]
fn real_build_produces_a_runnable_binary() {
    use huggr_toolkit::build::{BuildOptions, build};

    let (_, src) = scaffold_bundle("bcli-e2e", Template::Blank);
    let def = AgentDefinition::load(&src).unwrap();
    let out = std::env::temp_dir().join(format!("huggr-bcli-out-{}", std::process::id()));
...
"#,
    ...
    std::fs::write(docs_src.join("huggr.toml"), manifest).unwrap();
...
fn real_mcp_serve_round_trips_over_stdio() {
    ...
    let def = AgentDefinition::load(&src).unwrap();
    let out = std::env::temp_dir().join(format!("huggr-bcli-mcp-out-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&out);
    let binary = build(
        &def,
        &BuildOptions {

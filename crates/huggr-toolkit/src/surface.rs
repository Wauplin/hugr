...
//!
//! A binary produced by `huggr build` embeds its agent bundle as a
...
//! back as `status: "error"` answers (the `huglet-docs` contract, now universal).
...
const MANIFEST_NAME: &str = "huggr.toml";
...
/// iterator. Used by built binaries, embedded Rust agents, and `huggr run`.
...
fn surface_command(def: &AgentDefinition) -> Command {
    let mut command = Command::new(leak_str(def.agent.name.clone()))
        .about("A huglet. Ask it a question; errors are status:error answers.")
        .arg(
            Arg::new("trace")
                .long("trace")
...
pub fn prepare_home(bundle_bytes: &[u8]) -> std::io::Result<PathBuf> {
    let manifest = bundle::get(bundle_bytes, MANIFEST_NAME)?.ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::InvalidData, "bundle has no huggr.toml")
    })?;
    let name = manifest_identity(&manifest);
    let home = agent_home_dir(&name);
    std::fs::create_dir_all(&home)?;
    let hash = huggr_replay::BlobStore::hash(bundle_bytes).replace(':', "-");
    let cache_base = home
        .parent()
...
mod tests {
    ...
    fn bundle_for(template: Template, name: &str) -> (Vec<u8>, PathBuf) {
        let root =
            std::env::temp_dir().join(format!("huggr-surface-{name}-{}", std::process::id()));
    ...
    fn config_json_shows_key_env_name_but_never_the_secret() {
        ...
        unsafe { std::env::remove_var("HUGGR_CFG_TEST_KEY") };
    }
...
"#;
    ...
    async fn prepare_home_unpacks_and_agent_describes() {
        ...
        let home = std::env::temp_dir().join(format!("huggr-home-desc-{}", std::process::id()));
        let _ = std::fs::remove_dir_all(&home);
        // Point the home dir at a temp location via the runtime resolver.
        unsafe { std::env::set_var("HUGGR_AGENT_HOME", &home) };
        let prepared = prepare_home(&bytes).unwrap();
        assert_ne!(prepared, home);
        assert!(prepared.join("huggr.toml").exists());
        assert!(!home.join("huggr.toml").exists());
        ...
        unsafe { std::env::remove_var("HUGGR_AGENT_HOME") };
    ...
    async fn feedback_mode_appends_for_existing_trace() {
        ...
        let trace_id = traces
            .put(
                huggr_replay::Trace::new(Vec::new(), Vec::new(), Some(1)),

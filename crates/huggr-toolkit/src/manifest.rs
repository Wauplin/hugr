//! The `huggr.toml` manifest.
//!
//! A huglet definition is an auditable Rust crate folder: a `Cargo.toml`, a
//! `huggr.toml` manifest, and a `SYSTEM.md` system prompt beside it. This module
...
//! contract); `huggr run` assembles a `huggr-agent` runtime from it.
...
pub const MANIFEST_FILE: &str = "huggr.toml";
/// The Cargo manifest expected beside `huggr.toml` in an agent crate folder.
...
/// A parsed huglet definition. Produced by [`AgentDefinition::load`] (a
...
pub enum ToolKind {
    ...
    Mcp,
    /// Another Huggr agent granted as a tool (`[tools.agent.<name>]`).
    Agent,
}
...
pub enum ManifestError {
    /// No `huggr.toml` in the folder.
    #[error("no {MANIFEST_FILE} found in {dir}")]
    NotFound { dir: PathBuf },
    /// No `Cargo.toml` beside `huggr.toml`.
...
impl AgentDefinition {
    /// Load an agent crate folder: `<dir>/Cargo.toml` and `<dir>/huggr.toml`
...
/// (`huggr-agent`) consumption — the rest of the stack speaks JSON `Value`.
...
"#,
    ...
    fn tempdir() -> TempDir {
        ...
        let path =
            std::env::temp_dir().join(format!("huggr-toolkit-manifest-{}-{n}", std::process::id()));

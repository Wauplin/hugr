...
//! - every persisted trace replays bit-for-bit (`huggr_replay::verify`), so the
...
async fn answer_hook_runs_at_the_final_return_boundary() {
    ...
    huggr_replay::verify(&trace).unwrap();
}
...
async fn follow_up_writes_a_child_and_leaves_the_parent_untouched() {
    ...
    huggr_replay::verify(&store.get(&child.trace_id).unwrap()).unwrap();
}
...
async fn three_way_fork_root_t1_t2a_t2b() {
    ...
    for head in listed {
        huggr_replay::verify(&store.get(&head.trace_id).unwrap()).unwrap();
    }
...
fn tempdir() -> TempDir {
    ...
    let path = std::env::temp_dir().join(format!("huggr-agent-test-{}-{n}", std::process::id()));

//! `huggr-agent` — the common huglet runtime.
//!
//! This crate turns "an engine + a trace dir + a config" into a callable
//! huglet with a uniform contract: [`Ask`] in, [`Answer`] out. It is an
//! **internal layer**: the supported way to assemble an [`Agent`] is a
//! definition folder through `huggr-toolkit`'s `build_agent` (`huggr run` / a
...
/// `huggr-replay` so orchestrators can resolve an
/// [`Answer`] blob's `sha256` ref via [`Agent::blob_store`].
pub use huggr_replay::BlobStore;

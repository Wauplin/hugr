...
use baton_core::{ContentPart, ContextBlock, ModelRequest, OpId, Role, SamplingParams, StopReason};
...
data: [DONE]\n";
    ...
    while let Some(event) = rx.recv().await {
        if let baton_core::Event::ModelDelta {
            delta: baton_core::ModelDelta::Text(t),
            ..
        } = event
        {
            streamed.push_str(&t);

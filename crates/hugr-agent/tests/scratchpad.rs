...
use hugr_agent::{Agent, Ask, TraceId, TraceStore};
...
async fn nothing_escapes_the_scratch_root() {
    ...
    let agent = agent(
        ...
        vec![
            ModelOutput::tool_calls(vec![
                ...
                write_call("c2", "/tmp/hugr-escape.txt", "pwn"),
            ]),
    ...
    assert!(!std::path::Path::new("/tmp/hugr-escape.txt").exists());
}
...
fn tempdir() -> TempDir {
    ...
    let path = std::env::temp_dir().join(format!("hugr-scratch-test-{}-{n}", std::process::id()));

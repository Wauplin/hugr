...
pub fn browser_capabilities() -> Vec<BrowserCapability> {
    vec![
        ...
        mutating(
            "file_download_url",
            "Download a URL into Hugr's extension-local file store.",
            object(&[
        ...
        read_only(
            "file_list_downloads",
            "List files downloaded into Hugr's extension-local file store.",
            object(&[]),
        ),
        read_only(
            "file_read_metadata",
            "Read metadata for a file in Hugr's extension-local file store.",
            object(&[string("file_id", "The local Hugr file id.")]),
        ),
        mutating(
            "file_upload_to_input",
            "Upload a Hugr-local file to a page file input.",
            object(&[
                integer("tab_id", "The Chrome tab id."),
                string("file_id", "The local Hugr file id to upload."),
                string(
                    "node_id",
                    "Preferred snapshot node id. Leave empty when using selector.",
                ),
        ...
        mutating(
            "file_delete",
            "Delete a file from Hugr's extension-local file store.",
            object(&[string("file_id", "The local Hugr file id.")]),
        ),
    ]
}

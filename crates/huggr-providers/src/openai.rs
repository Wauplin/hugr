...
use huggr_core::{ContentPart, ModelOutput, ModelRequest, Role, ToolCall, Usage};
...
// Point `HUGGR_BASE_URL` at `https://api.openai.com/v1` to use OpenAI directly.
...
mod tests {
    use super::*;
    use huggr_core::{ContextBlock, Event, ModelDelta, ModelRequest, Role, ToolSchema};
    use huggr_host::ModelSink;
    ...
    async fn accumulator_consolidates_text_and_usage() {
        ...
        let sink = ModelSink::new(huggr_core::OpId(0), tx);

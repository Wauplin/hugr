...
use hugr_core::{OpOutcome, Record, ToolSchema, Value};
use hugr_host::{Capability, ChunkSink};
use hugr_replay::Trace;
use serde_json::json;

use hugr_agent::{FeedbackBackend, FsFeedbackStore, TraceId, TraceStore};
...
impl TracesRoot {
    ...
    /// expanded against `$HOME` so grants can name `~/.hugr/<agent>` directly.
...
mod tests {
    use super::*;
    use hugr_agent::{Feedback, TraceHeader};
    ...
    fn fixture_home() -> (TempDir, TraceId) {
        ...
        let id = store
            .put(
                ...
                TraceHeader::new("fixture", "0.0.1", "what is hugr?", "success"),
            )
            .unwrap();
        (home, id)
    }
    ...
    async fn trace_list_reports_heads_and_feedback_counts() {
        ...
        assert_eq!(result["traces"][0]["question"], "what is hugr?");
    }

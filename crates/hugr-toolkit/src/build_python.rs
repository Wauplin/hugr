...
//!   python/<module>/
//!     __init__.py        # typed ask(...) -> Answer, re-exports the models
//!     _models.py         # generated dataclasses: <Response>, Answer, AnswerMeta…
...
pub struct PythonBuildOutcome {
    ...
    /// The importable Python module name (e.g. `hugr_docs`).
    pub module: String,
...
/// is a legal Python module name (e.g. `hugr-docs` → `hugr_docs`).
...
/// paths back to the installed `hugr-toolkit` and (for a typed contract) the
...
/// Run exactly one ask and return the `Answer` as a JSON string. Errors are
...
fn _native(m: &Bound<'_, PyModule>) -> PyResult<()> {{
    m.add_function(wrap_pyfunction!(ask_json, m)?)?;
    Ok(())
}}
"#
    )
...
class BlobHandle:
    ...
    @classmethod
    def _from_dict(cls, d: dict) -> "BlobHandle":
        return cls(
            ref=d["ref"],
...
class AnswerMeta:
    ...
    @classmethod
    def _from_dict(cls, d: dict) -> "AnswerMeta":
        return cls(
            duration_ms=d.get("duration_ms", 0),
            cost_micro_usd=d.get("cost_micro_usd", 0),
...
class Answer:
    ...
    def _from_dict(cls, d: dict) -> "Answer":
        ...
        return cls(
            ...
            metadata=AnswerMeta._from_dict(d.get("metadata") or {{}}),
            response=response,
            error=error,
            blobs=[BlobHandle._from_dict(b) for b in d.get("blobs", [])],
            extra=d.get("extra"),
        )
"#,
...
/// The generated `__init__.py`: the typed public `ask(...)`. Declared runtime
/// arguments become typed parameters (positional ones lead, before the
/// question), mirroring the CLI surface.
fn init_py(def: &AgentDefinition, module: &str, root_class: &str) -> String {
    ...
    format!(
        r#"# Generated by `hugr build --surface python`. Do not edit by hand.
"""Typed Python bindings for the `{agent_name}` Hugr subagent.
...
__all__ = ["ask", "Answer", "AnswerMeta", "BlobHandle", "{root_class}"]
...
{doc}    """
{runtime_build}    raw = _native.ask_json(
        question,
        trace_id,
...
"#;
        AgentDefinition::parse(src, "hugr.toml").unwrap()
    }

    #[test]
    fn module_name_is_python_legal() {
        assert_eq!(python_module_name("hugr-docs"), "hugr_docs");
    ...
    fn cargo_toml_declares_cdylib_and_pyo3() {
        let toml = cargo_toml("hugr-docs", &None);
    ...
    fn pyproject_uses_maturin_mixed_layout() {
        let py = pyproject_toml(&docs_def(), "hugr_docs");
        assert!(py.contains("build-backend = \"maturin\""));
        assert!(py.contains("python-source = \"python\""));
        assert!(py.contains("module-name = \"hugr_docs._native\""));
        assert!(py.contains("name = \"hugr-docs\""));
        assert!(py.contains("version = \"0.0.2\""));
    }

    #[test]
    fn lib_rs_bridges_to_ask_bundle() {
        let rs = lib_rs(&None);
        assert!(rs.contains("fn ask_json("));
    ...
    fn init_py_exposes_positional_runtime_arg_before_question() {
        let init = init_py(&docs_def(), "hugr_docs", "DocsResponse");

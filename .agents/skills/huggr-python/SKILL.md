...
`Agent.ask()` and `Agent.run()` share the signal-aware native event bridge. `Ctrl+C` aborts a blocking ask, while task cancellation or iterator close aborts a stream. An in-process Python callable already executing on a blocking worker cannot be preempted safely and may finish after cancellation; keep its side effects idempotent or implement application-level cancellation. Runtime teardown does not wait for it. Generated wheels for Rust-defined agents expose module-level `ask(...)` and `run(...)` methods with the same parameters and event dataclasses; their successful `Answer.response` remains the generated Rust response-contract dataclass.

...
- **T1.6 — redefine `hugr-docs` as a definition ✅.** `crates/hugr-docs/definition/` is now the source of truth for the docs agent: `hugr.toml` grants the toolkit `fs_read` library jailed to the docs root, declares the `docs` model tier/pricing/default trace store, and carries the `Answer.extra` schema for `{ answer, related_documents }`; `SYSTEM.md` is the docs retrieval prompt rewritten for the generic `fs_*` tools. The `hugr-docs` crate no longer assembles a bespoke `Agent`/provider/tool set directly: CLI/Python options are converted into runtime overrides on that checked-in `AgentDefinition` (docs root, model/base URL, pricing, trace dir, version), then the crate calls `hugr_toolkit::runtime::build_agent_with_provider_key` and the shared `hugr-agent` ask path. The toolkit gained that explicit provider-key override so compatibility surfaces can pass secrets without mutating process-global env. The legacy output contract remains crate-owned (`status`/`message`/`trace_id`/`related_documents`/`metadata`) and its read accounting recognizes both old `docs_*` traces and new definition-run `fs_*` traces, preserving existing trace/test corpus behavior while new runs come from the definition. `crates/hugr-docs/README.md` documents the definition folder and generic toolkit tools. Tests: `hugr-docs` pins that the embedded definition parses and receives the surface overrides, that definition-run `fs_*` tool results produce the same related-document accounting as historical `docs_*` results, and all existing answer-shape/config/error tests still pass. Verification run: `cargo test -p hugr-docs --offline` (18 passed); `cargo test -p hugr-toolkit --offline` (55 passed, 5 ignored); `cargo clippy -p hugr-docs -p hugr-toolkit --all-targets --offline` (clean).
...
**Phase T1 complete.** `hugr new` → edit config → `hugr run` gives a working, sandboxed, trace-persisting subagent with zero Rust, traces are inspectable/verifiable via `hugr traces`/`replay`/`verify`, and `hugr-docs` is now backed by a checked-in definition folder run through the same toolkit runtime.
...
- **T2.2 — Rust-crate surface ✅.** `hugr build --surface crate` emits a **library** crate exposing the typed `hugr_agent::Agent` for direct embedding (ARCHITECTURE §21.2) — Rust orchestrators skip serialization entirely. Shared plumbing: `surface::load_agent(bundle) -> Result<LoadedAgent, LoadError>` (prepare-home → `AgentDefinition::load` → `build_agent`, collecting warnings) is now the one "embedded bundle → callable agent" path behind **both** the CLI surface (`run_cli` refactored onto it) and the generated crate; `prepare_home`/`LoadError`/`LoadedAgent` are public. `build.rs` gained a `Surface::Crate` variant, a `build(def, surface, opts)` dispatcher, and `build_crate`, which generates a workspace-detached lib crate (`[lib]` with the underscored name, path deps to both `hugr-toolkit` and `hugr-agent`, `src/lib.rs` = `include_bytes!` the bundle + re-export the `hugr_agent` contract types + `pub async fn load()`/`ask()`) and `cargo check`s it. `BuildOutcome.binary` is now `Option` (a crate surface has no binary). The shim-generation is factored (`write_bundle` + `run_cargo` shared by cli/crate). `hugr build --surface crate` prints the generated crate path and a depend-on-it hint. Tests: `crate_cargo_toml` is a lib with both path deps and no `[[bin]]`; the `#[ignore]`d `real_crate_surface_compiles` generates the crate, `cargo check`s it, **and** builds a downstream consumer crate that path-depends on it and calls `bcli_crate::ask(..)`/`load()` — the exact exit criterion (passed in ~32s). Verification run: `cargo test -p hugr-toolkit --offline` (36 lib + integration passed) + `cargo clippy` (clean but the pre-existing T1.4 scaffold lint); the ignored crate-surface e2e passed (~32s).

- **T2.3 — Python surface ✅.** `hugr build --surface python` emits a **maturin/PyO3 package** exposing `answer(question, trace_id=None, blobs=None) -> dict` plus `describe()`/`traces()` (ARCHITECTURE §21.3) — the `hugr-docs` binding generalized. `build.rs` gained `Surface::Python` + `build_python`, generating a workspace-detached `cdylib` crate: `Cargo.toml` (path deps to `hugr-toolkit` + `hugr-agent`, `serde`/`serde_json`/`tokio`, `pyo3` with `extension-module`+`abi3-py39`), a maturin `pyproject.toml`, and `src/lib.rs` (a `__HUGR_MODULE__` marker replaced with the module name). The generated `answer` reuses `surface::load_agent`, builds the `Ask` (blobs via the now-public `surface::blob_handle_from_path`), runs it on a fresh current-thread tokio runtime inside `allow_threads`, and **never raises for run failures** — a `LoadError`/`AskError`/bad-blob becomes a `status: "error"` Answer dict (serialized via `json.loads`), so Python callers branch on `result["status"]`; `describe()`/`traces()` are audit views that may raise on a load failure. `build_python` `cargo check`s the crate (extension-module is link-free under `check`). The `hugr build` CLI prints a surface-aware next-step (the `maturin build` incantation for python). Tests: `python_generators_are_a_pyo3_cdylib` pins the generated `Cargo.toml`/`pyproject.toml`/`lib.rs` shape + marker substitution; `#[ignore]`d `real_python_surface_compiles` generates + `cargo check`s the extension module. **Full exit criterion verified manually** (maturin 1.14 + Python 3.10 present in this env): `maturin build --release` produced an abi3 wheel, `pip install`ed into a venv, and from Python `describe()["name"]` returned the agent name, `answer("…")` returned a `status: "error"` dict with a persisted `trace_id` (no model reachable — never raised), `answer("…", trace_id=…)` produced a **new child trace**, and `traces()` listed both — resume-from-Python proven. Verification run: `cargo test -p hugr-toolkit --offline` (37 lib + integration passed); `cargo clippy` (clean but the pre-existing T1.4 scaffold lint); the ignored python-surface compile test (~17s) + the manual wheel/pip/answer round-trip.

- **T2.4 — MCP server surface ✅.** A cli-built agent binary run with `--mcp-serve` speaks the Model Context Protocol over stdio (newline-delimited JSON-RPC 2.0), exposing a single `ask` tool (ARCHITECTURE §21.4) — how Claude Code / other orchestrators consume a Hugr agent natively. New `src/mcp_serve.rs`: `serve(agent, card)` reads stdin lines via the reused `hugr_plugin_abi::framing` helpers and dispatches each message through a transport-free `handle_message(agent, card, msg) -> Option<Value>` (None for notifications): `initialize` (capabilities `{tools:{}}` + serverInfo/instructions from the `AgentCard`, echoing the client's `protocolVersion`, default `2024-11-05`), `tools/list` (the `ask` schema — `question` required, optional `trace_id`, optional `blobs`), `ping`, and `tools/call` for `ask` (deserializes args incl. contract `BlobHandle`s, runs `Agent::ask`, returns the full `Answer` as `structuredContent` + a text block, `isError:false` since run failures are already `status:"error"` answers; infra `AskError` → JSON-RPC `-32602`); unknown methods → `-32601`. Per the stateless MCP design, **session continuity rides `trace_id` in the tool arguments, not MCP session state** — a follow-up is another `ask` carrying the prior answer's `trace_id`; we never use MCP sampling. `build.rs` gained `Surface::Mcp`, which builds the **same** cli binary (the mode is a built-in flag) and prints the `--mcp-serve` registration hint; `--mcp-serve` is a `SurfaceArgs` flag on the universal CLI, dispatched before the audit/ask modes. Tests: `mcp_serve` unit tests pin the `ask` schema, the `initialize` result, and the JSON-RPC envelopes; `protocol_round_trips_ask_and_resume` drives the full dispatch against a real in-process agent (notification→no reply, initialize, tools/list, ask→persisted `trace_id`, resume→new child trace, unknown tool→`-32602`); the `#[ignore]`d `real_mcp_serve_round_trips_over_stdio` builds the binary and drives `--mcp-serve` over **real stdio pipes** (initialize + two ask calls with `trace_id` round-tripping — the exit criterion, passed in ~23s). Verification run: `cargo test -p hugr-toolkit --offline` (46 passed, 4 ignored) + `cargo clippy` (clean but the pre-existing T1.4 scaffold lint); the ignored stdio e2e passed (~23s).

- **T2.5 — surface conformance suite ✅.** `crates/hugr-toolkit/tests/conformance.rs` runs one scripted scenario — **ask → follow-up → fork → describe** — against **every** surface of the *same* definition and asserts they produce an identical normalized result (ARCHITECTURE §21), the gate for `hugr build` changes. Because every surface is a thin serialization of the one `hugr-agent` API, they must agree on a `Normalized { describe_name, sorted tool names, per-ask statuses, distinct_traces }`: the agent card, each ask's status, and the trace-lineage invariant that a follow-up and a fork each write fresh distinct trace ids (checked without a model — offline, every ask is a `status:"error"` answer but a trace still persists, so lineage holds identically). The **reference** leg is the in-process typed `Agent` (the `crate` surface's semantics, serialized through the wire form for field-parity); the **cli** and **mcp** legs drive one built binary (`--json` invocations / `--mcp-serve` stdio JSON-RPC — the mcp leg also asserts `serverInfo.name` matches the card); the **python** leg (maturin-gated: skipped-with-a-log when maturin/python3 are absent, no silent cap) builds a wheel, pip-installs it into a venv, and runs `describe()`/`answer()`/resume from Python. The suite is `#[ignore]`d (it compiles real artifacts). **Verified end-to-end in this environment** (maturin + python3 present): `cargo test -p hugr-toolkit --test conformance -- --ignored` passed in ~102s with all four surfaces (crate/cli/mcp/python) agreeing. Verification run: `cargo test -p hugr-toolkit --offline` (46 passed, 5 ignored); `cargo clippy -p hugr-toolkit --all-targets --offline` (**No issues found** — the long-standing scaffold lint is also fixed); the ignored conformance suite passed (~102s).

**Phase T2 complete.** One definition folder ships as a binary (`--surface cli`), a typed Rust crate (`--surface crate`), a Python wheel (`--surface python`), and a stdio MCP server (`--surface mcp` / `--mcp-serve`) — the same ask/answer contract on each, proven by the conformance suite. Every artifact embeds its whole definition (via the deterministic `bundle`) and needs no repo checkout to run. **T1.6 (redefine `hugr-docs` as a definition folder) remains deferred** pending the config-override surface (T3.5) — the docs agent's `fs_read` root is a runtime parameter a fixed definition folder can't yet express.

## Roadmap T3 — Orchestration hardening

- **T3.1 — limits enforcement ✅.** `hugr-agent` now enforces the declared `AgentLimits` host-side on every ask (ARCHITECTURE §20.1, ROADMAP T3.1), leaving the sans-IO core untouched. New `src/limits.rs`: the **counting/cost** bounds (`max_model_calls`, `max_turns`, `max_cost_micro_usd`) are enforced by wrapping each registered model adapter in a `LimitedAdapter` sharing an `Arc<LimitState>` (atomic call counter + running cost accumulator, priced from the same per-tier `Pricing` used for accounting) — before each model call it checks the bound and, once crossed, **refuses** the call by returning an error, which the brain folds into `ModelError` → `Done(Error)` and ends the turn; the cost bound refuses the *next* call once the running total of completed calls crosses it (a call's cost isn't known until it returns usage). The **wall-clock** bound (`timeout_ms`, from the manifest's `timeout_s`) wraps the turn future in `tokio::time::timeout` in the ask path; on elapse the turn future is dropped mid-flight (the recorded event prefix stays self-consistent) and the trip is recorded. Either way the ask returns a normal error **answer** — never an `AskError`: `status: error`, the human message in `message`, and a typed machine-readable reason on `Answer.extra` (`{"limit_exceeded": {"limit": "max_model_calls"|"max_turns"|"max_cost_micro_usd"|"timeout_ms", "value": <bound>}}`), plus a persisted `trace_id`. The partial trace still replays bit-for-bit because a refusal/timeout only ever shortens the recorded event stream (`verify()` re-feeds recorded events and never re-calls the adapter). Enforcement is zero-overhead when no limit is set (adapters unwrapped, no timeout). `hugr-agent` gained `tokio` (time) + `anyhow` as normal deps (host-layer, core stays sans-IO); `Pricing::cost_micro_usd` is now `pub(crate)`; `LimitKind` is re-exported. The toolkit already maps `[limits]` → `AgentLimits` in `runtime::build_agent`, so every `hugr run`/built-surface agent gets enforcement for free. Tests (`crates/hugr-agent/tests/limits.rs`, real tokio engine + a looping mock tool / a slow mock model, no network): each of the four limits trips cleanly as an error answer with the right `extra` reason and correctly-billed `metadata` (2 model calls under `max_model_calls=2`; cost 58 stopping under `max_cost_micro_usd=40`; etc.), each persisted partial trace `verify()`s, and a no-limits agent is unchanged (success answer, null extra). Verification run: `cargo test -p hugr-agent --offline` (31 passed); `cargo test -p hugr-toolkit --offline` (46 passed, 5 ignored); `cargo clippy -p hugr-agent --all-targets --offline` (clean).

- **T3.8 — agent-as-tool grants ✅.** A `[tools.agent.<name>]` grant now registers a child Hugr agent as an ordinary `agent_<name>` capability (ARCHITECTURE §20.5, ROADMAP T3.8). `hugr-agent` gained `src/agent_tool.rs`: an `AgentTool` capability wrapping an `AgentToolResolver` (an async closure the host supplies to run the child), plus `AgentToolSpec` and `AgentBuilder::agent_tool`. Its schema takes an `Ask` (question + optional `trace_id` for follow-ups/forks); its result is the child's full `Answer`. `Agent::ask` registers each agent-tool **fresh per ask** with a shared spend sink, and after the turn folds every child `Answer.metadata` into the parent's via the new `AnswerMeta::merge_child` (tokens/cost/model-calls/tool-calls + per-tier merge by selector), so the parent's reported cost includes the child (§18.4). `describe()` lists agent-tools under a new `ToolPrivilege::Agent`. The toolkit wires it in `build_agent`, now depth-aware (`build_agent_depth(def, agent_depth)`, default `DEFAULT_MAX_AGENT_DEPTH = 3`): a `ref` to a **definition folder** runs the child in-process (recursive build with `agent_depth − 1`), a `ref` to a **file** runs it as a subprocess artifact over the CLI JSON contract (`run_subprocess_agent`), and at zero remaining depth the grant becomes a `depth_exceeded_resolver` stub — which **statically cuts cycles** (`a`→`b`→`a`→… terminates at the cap). A bad `ref` is a `RuntimeError::Agent` build error. **v1 scope (documented in §20.5):** the child session is recorded as the tool's opaque result + the child's own standalone trace (resumable via `trace_id`), and the parent trace `verify()`s bit-for-bit (the capability isn't re-run on replay); physically nesting a `ChildTrace` and forwarding resource-group *resources* into the child are deferred to the T4 demo. Tests: `crates/hugr-agent/tests/agent_as_tool.rs` (real engine + mock models — a parent delegates to a child, the child's cost folds in exactly `parent 58 + child 20 = 78` microUSD with merged per-tier line, a follow-up via the returned child `trace_id` resumes the child thread with correct `depends_on`, and the parent trace verifies); toolkit tests for interpreter-grant registration on the card, cyclic definitions terminating via the depth cap, and a bad `ref` → build error. Verification run: `cargo test -p hugr-agent --offline` (44 passed); `cargo test -p hugr-toolkit --offline` (55 passed, 5 ignored); `cargo clippy -p hugr-agent -p hugr-toolkit --all-targets --offline` (clean).

- **T3.7 — resource groups & grants ✅.** Orchestrator-supplied resource grants are now a typed, deterministic part of the contract (ARCHITECTURE §18.5, ROADMAP T3.7). `hugr-agent`'s contract gained `ResourceRef` (`FsRoot`/`Blob`/`Sqlite`/`HttpHost`, tagged snake_case), `ResourceGroup { name, resources }`, `Access { Read, ReadWrite }` (with `satisfies` — `ReadWrite` covers both), and `ResourceGrant { group, access }`; `Ask` gained `groups` + `grants` slots (serde-defaulted/skipped-when-empty, so the minimal wire ask is unchanged), and the committed `ask.schema.json` + contract lockstep/wire-snapshot pins were extended in step. A manifest tool scope may **bind a group** instead of a concrete path (`[tools.fs_read] root = "group:policies"`); the toolkit's `tools::group_scope` detects that, and `build_agent` registers a **`GroupBinding`** (new `AgentBuilder::group_binding`) instead of an eager capability — a `GroupCapabilityFactory` that builds the tool's capabilities from the granted group's resources (`fs_read` from an `FsRoot`, `sqlite_query` from a `Sqlite`). In `Agent::ask`, `resolve_group_bindings` registers a bound tool **only** when the ask carries a grant of sufficient access over its group (sandbox-by-registration extended to caller scopes): no grant → never registered (silent, by design); insufficient access → skipped with an advisory `Answer.warnings` notice; a `Read` grant satisfies read-class tools, `ReadWrite` additionally satisfies write-class ones. Effective grants are the ask's own if it supplies any, else the parent trace's recorded set — so a plain resume **re-derives the identical registration from the trace alone**; grants+groups are recorded in the opaque `TraceMeta.grants` slot (new, serde-defaulted — pre-T3.7 traces load/serialize unchanged) and in `TraceHeader`, and a fork that changes grants records the new fact without mutating the parent. (Known v1 seam: on resume the model-*advertised* schema set follows the resumed policy per the engine's resume semantics, while execution-time registration follows the new grant; documented in the test.) Tests: `crates/hugr-agent/tests/resource_groups.rs` (a `ToolSpy` mock model observes `ModelRequest.tools` — Read registers only the reader, ReadWrite both, no grant neither; resume with no new grants re-derives ReadWrite; a fork downgraded to Read records `access: "read"` while the parent stays `"read_write"`); toolkit `group_scope` detection + a `build_agent` test proving a `group:`-bound `fs_read` is absent from the agent card without a grant while scratch tools remain. Verification run: `cargo test -p hugr-agent --offline` (43 passed); `cargo test -p hugr-toolkit --offline` (53 passed, 5 ignored); `cargo test -p hugr-replay --offline`; `cargo clippy -p hugr-agent -p hugr-toolkit -p hugr-replay --all-targets --offline` (clean).

- **T3.6 — sandbox tightening pass ✅.** Reviewed every library tool's jail against its escape vectors and wrote the per-tool threat model at `docs/THREAT_MODEL.md` (ARCHITECTURE §20.2, ROADMAP T3.6). Findings + fixes: **`http_fetch`** followed HTTP redirects by default while the allowlist was only checked on the *initial* URL — an allowlisted host could `3xx`-redirect to an off-allowlist/internal target (SSRF bypass). Fixed by disabling automatic redirects (`reqwest::redirect::Policy::none()`), so a `3xx` returns to the model as-is and following it is a new, re-checked `http_fetch` call; regression tests pin that a userinfo trick (`https://allowed@evil.com`) and non-`http(s)` schemes (`file://`) can't bypass the allowlist, and that suffix-collision hosts (`notexample.com`) don't match. **`sqlite_query`**'s `ATTACH` guard was a substring scan (false-positive on a column named `attachment`, and the sole second-file control); replaced with a **token-based** `mentions_attach_keyword` (whole SQL word, any casing/spacing) with unit tests for both real ATTACH and identifier false-positives (the module stays behind the `sqlite` feature, so these run when it's enabled). **`fs_read`**'s jail was already sound (component check + post-canonicalize `starts_with(root)` re-check); added a unix regression test proving a symlink *inside* the root pointing *outside* is rejected by the canonical re-check (its path components are all `Normal`, so the component check alone wouldn't catch it). The threat-model doc also records the **scratchpad** jail (canonicalized parent on write, relative-only results), documented residuals (read-only TOCTOU, DNS-rebinding/internal-IP SSRF, the SQL-text-vs-engine-authorizer tradeoff), and that **mcp/plugin** grants are process-boundary trust decisions surfaced by `--config`. ARCHITECTURE §20.2 links the doc. Verification run: `cargo test -p hugr-toolkit --offline` (51 passed, 5 ignored); `cargo clippy -p hugr-toolkit --all-targets --offline` (clean).

- **T3.5 — config/env audit surface ✅.** `<agent> --config` now prints the effective configuration with **real per-key provenance and secret redaction** (ARCHITECTURE §18.2, ROADMAP T3.5). Provenance can only be known by the layer that assembled the agent, so `hugr-agent` gained an `AgentBuilder::config_entries(Vec<ConfigEntry>)` channel: when supplied, `Agent::config()` returns those verbatim (else it derives the old builder/default-tagged view — the T0.7 fallback stays for direct API users). The toolkit's new `runtime::effective_config(def, base_dir) -> Vec<ConfigEntry>` builds the annotated list and `build_agent` wires it: manifest-present values are tagged `Manifest`, fallbacks `Default`, and the **provider secret is resolved from its env var (`Env`) and redacted** — the manifest only ever carries the var *name* (`models.api_key_env`, surfaced as non-secret), a non-secret `models.api_key_resolved` bool records whether it resolved, and `models.api_key` is a `ConfigEntry::redacted` whose value is never the key. Entries cover identity, models (base_url/tiers/default), each tool grant (kind + scope — the blast-radius audit), limits, resolved scratch/trace paths, and any declared answer-extra schema, in deterministic order. `Flag` provenance is reserved for the config-override surface (T1.6/T2.3 kwargs). Tests (`runtime.rs`): `effective_config` tags manifest keys `Manifest`, an undeclared description `Default`, surfaces the api-key env var name while proving the secret entry is `redacted`+`Env` and no entry leaks a key value. Verification run: `cargo test -p hugr-toolkit --offline` (48 passed, 5 ignored); `cargo test -p hugr-agent --offline` (41 passed); `cargo clippy -p hugr-agent -p hugr-toolkit --all-targets --offline` (clean).

- **T3.4 — structured answer extras ✅.** A manifest-declared JSON schema for `Answer.extra`, validated post-hoc, violations advisory (ARCHITECTURE §18.1/§20.1, ROADMAP T3.4). New `hugr-agent/src/answer_schema.rs`: a deliberately **minimal, dependency-free** JSON-Schema-subset validator (`validate_extra(schema, value) -> Vec<String>`) covering `type` (object/array/string/number/integer/boolean/null), `required` + `properties` for objects, and `items` for arrays (recursively); unknown keywords are ignored, matching the "advisory, never load-bearing" intent. `AgentBuilder::answer_schema(Value)` declares the schema; in `Agent::ask`, when a schema is set and the run produced a plain (non-error) answer, the final JSON message is **lifted into `extra`** and validated — conforming extras attach cleanly, violations become `Answer.warnings` and **never fail the ask**. `Answer` gained a `warnings: Vec<String>` slot (advisory, `#[serde(default, skip_serializing_if = Vec::is_empty)]` so the wire form is unchanged for callers that emit none) with `with_warnings`; the committed `answer.schema.json` + the `tests/contract.rs` lockstep/wire-snapshot pins were updated in step (the sanctioned non-breaking evolution of a `#[non_exhaustive]` contract type). Toolkit: a new `[answer]` manifest section (`AnswerConfig { extra_schema?, extra_schema_file? }`, inline TOML table → `serde_json::Value` or a `.json` path) parsed by `manifest.rs` (known-key-checked, warns on typos) and wired in `runtime::build_agent` via `resolve_answer_schema` (inline preferred, else read the file relative to the definition folder; a read/parse failure is a non-fatal build warning since the schema is advisory). Reference manifest + ARCHITECTURE §20.1 document the block. **Docs-agent note:** the mechanism generalizes `hugr-docs`' `related_documents`; moving the docs agent's extra onto a declared schema lands with the deferred T1.6 port (the crate still does its own post-processing today). Tests: `answer_schema.rs` unit tests (conforming/missing-required/bad-item-type/type-mismatch/unknown-keyword); `crates/hugr-agent/tests/answer_extra.rs` (real engine + JSON-emitting mock model — a conforming message lifts into `extra` with no warnings; a message missing the required field yields exactly one warning while the ask still `status: success`); a manifest parse test for inline `[answer.extra_schema]`; contract lockstep tests updated for `warnings`. Verification run: `cargo test -p hugr-agent --offline` (41 passed); `cargo test -p hugr-toolkit --offline` (47 passed, 5 ignored); `cargo clippy -p hugr-agent -p hugr-toolkit --all-targets --offline` (clean).

- **T3.3 — trace lifecycle ✅.** `TraceStore` gains `prune(&PrunePolicy) -> PruneReport` and `size() -> StoreSize` (ARCHITECTURE §19.4, ROADMAP T3.3). `PrunePolicy` carries three optional bounds — `keep_max` (keep the N most-recently-modified, LRU by file mtime), `max_age_secs` (drop traces older than that by mtime), and `pinned` ids (never dropped) — and a trace is a drop-candidate if it fails **any** active bound and is not pinned. The invariant that matters is **lineage closure**: after computing the policy keep-set, every kept trace's ancestor chain (`depends_on`, transitively, within the store) is pulled back in, so a survivor's lineage always still resolves — pruning can never orphan a kept trace. Age/LRU use filesystem mtime deliberately (host maintenance, outside the deterministic replay path — traces themselves stay immutable and content-keyed). `PruneReport` reports pruned/kept ids (sorted) + bytes freed; `StoreSize` reports trace count + total `.json` bytes. `Agent::prune` wraps the store prune and additionally deletes each pruned trace's per-lineage `.scratch/<id>` subtree (blob-store GC stays separate — blobs are content-addressed and shared); `Agent::store_size` exposes the size report. The `hugr traces` CLI grew `--prune [--keep-max N] [--max-age-secs S] [--pin <id>]…` and `--size` (developer inspection commands: stdout + non-zero exit on failure). Tests (`crates/hugr-agent/tests/store.rs`, mtimes pinned via `File::set_times` for deterministic ordering): a `keep_max=1` prune of a `root → t1 → t2` chain plus an independent `root2` keeps the whole surviving chain (closure pulls in `t1`+`root`) and drops only `root2`, with the chain's `depends_on` still resolving end-to-end; a `max_age_secs=60` prune of three hour-old traces keeps a **pinned** leaf and its ancestor while dropping the unrelated stale one; and `size()` reports the right count/bytes. Verification run: `cargo test -p hugr-agent --offline` (35 passed); `cargo clippy -p hugr-agent --all-targets --offline` + `cargo clippy -p hugr-toolkit --all-targets --offline` (clean).

- **T3.2 — concurrent asks ✅.** The concurrency model is now documented and tested (ARCHITECTURE §19.2, ROADMAP T3.2): each `Agent::ask` is an **independent session** — it assembles its own engine, prepares its own `.pending/<pid>-<n>` scratch subtree (the per-agent monotonic counter already makes working dirs collision-free), and persists a **new** immutable trace; immutability makes forks race-free because a shared parent is only ever *read*. The one race that could bite parallel asks was trace-id assignment: `TraceStore::put` derived the id from content and collision-checked with a bare `path.exists()` before `save_atomic` — a TOCTOU window where two asks landing on the same content hash could both claim the same file and one clobber the other. `put` now **reserves the id's path atomically** with `OpenOptions::create_new(true)`: the loser sees `AlreadyExists` and deterministically bumps the `-N` suffix, so N parallel puts always yield N distinct ids and no trace is ever overwritten (the reserved empty placeholder is then replaced crash-atomically by `save_atomic`, whose temp name derives from the now-unique candidate so concurrent writes never share a temp path). Tests (`crates/hugr-agent/tests/concurrent_asks.rs`, real multi-thread tokio runtime + a shared agent/mock model): 16 asks race — even indices fresh roots, odd indices forks off one shared parent — and all 16 (plus the root) produce distinct trace ids, each with correct `depends_on` lineage, the store lists exactly N+1 traces, every one `verify()`s bit-for-bit, and the shared parent stays an unchanged root. Verification run: `cargo test -p hugr-agent --offline` (32 passed); `cargo clippy -p hugr-agent --all-targets --offline` (clean).

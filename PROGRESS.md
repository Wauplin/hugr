...
- `hugr-providers` now owns a host-side `TierModelConfigSet` for exactly `small`/`medium`/`big`, loaded from the `models` section of `HUGR_CONFIG` and overridden by `OPENAI_BASE_URL` / `OPENAI_MODEL` / CLI `--model`. All three tiers default to the same HF router model until better product defaults are chosen. `OpenAiAdapter` accepts per-tier default sampling knobs while request-level params still win.
